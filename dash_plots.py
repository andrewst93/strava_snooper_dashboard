...
from strava_data_load_preprocess import load_week_start_times_data, load_lgbm_model_results, load_logreg_model_results, load_lgbm_heatmap, \
                                        load_logreg_heatmap, load_lgbm_model, load_logreg_model


ACTIVITY_COLOR_MAP = { #Plotly COlors: ['#636EFA', '#EF553B', '#00CC96', '#AB63FA', '#FFA15A', '#19D3F3', '#FF6692', '#B6E880', '#FF97FF', '#FECB52']
    'AlpineSki'     : '#636EFA',
    'Ride'          : '#00CC96',
    'NordicSki'     : '#EF553B', 
    'Run'           : '#AB63FA',
...
def plot_lgbm_model_predictions(train_data, plot_data, labels, data_set, lgbm_model = None):
    ...
    try: 
        ...
        prediction = lgbm_model.predict(train_data, num_iteration=lgbm_model.best_iteration)
    ...
    data_plot = px.scatter(   x=plot_data['morn'], y=plot_data['aft'], 
                                    color=np.where(lgbm_results.prediction.round() == 1, 'unemployed', 'employed'), color_discrete_map={'unemployed':'#009FFD', 'employed': '#FFA400',}, 
                                    symbol= np.where(lgbm_results.prediction.round() == np.array(lgbm_results.label), 'Correct', 'Incorrect'),
    ...
    if len(train_data.iloc[0]) == len(plot_data.iloc[0]):
        try:
            ...
            lgbm_heatmap.to_csv(os.path.join('data',  data_set + '_lgbm_model_heatmap.csv'))

        data_plot.add_trace(go.Heatmap(x=lgbm_heatmap.X, y=lgbm_heatmap.Y, z=lgbm_heatmap.Z,
                        colorscale=[[0, '#FFA400'], [0.4, '#FFA400'], [0.6,'#009FFD'], [1,'#009FFD']],#[[0,'#FFA400'],  [1,'#009FFD']], #[0.45, '#009FFD'], [0.55,'#FFA400'],
                        opacity=0.3,
                        showscale=False,)
        )
...
def plot_logreg_model_predictions(data, labels, data_set, logreg_model = None):
    ...
    data_plot = px.scatter(   x=data['morn'], y=data['aft'], 
                                    color=np.where(logreg_results.label == 1, 'unemployed', 'employed'), color_discrete_map={'unemployed':'#009FFD', 'employed': '#FFA400',}, 
                                    symbol= np.where(logreg_results.prediction == np.array(logreg_results.label), 'Correct', 'Incorrect'),
                                    symbol_map= {'Correct':'circle',
                                                'Incorrect': 'x'},
                                    labels={
                                        'color':'Label',
    ...
    try:
        ...
        logreg_heatmap.to_csv(os.path.join('data',  data_set + '_logreg_model_heatmap.csv'))

    data_plot.add_trace(go.Heatmap(x=logreg_heatmap.X, y=logreg_heatmap.Y, z=logreg_heatmap.Z,
                    colorscale= [[0, '#009FFD'], [0.45, '#009FFD'], [0.55,'#FFA400'], [1,'#FFA400']],#[[0,'#FFA400'],  [1,'#009FFD']], #[0.45, '#009FFD'], [0.55,'#FFA400'],
                    opacity=0.3,
                    showscale=False,)
    )
...
def plot_training_data(train_data, train_labels, test_data, test_labels):
    ...
    x_min, x_max = combined_data['morn'].min() - 0.02, combined_data['morn'].max() + 0.02
    y_min, y_max = combined_data['aft'].min() - 0.02, combined_data['aft'].max() + 0.02

    data_plot = px.scatter(x=combined_data['morn'], y=combined_data['aft'], 
                                    color=combined_labels, color_discrete_map={'unemployed':'#009FFD', 'employed': '#FFA400'},
                                    labels={
                                        'color':'Label',
                                    }
                                ).update_traces(marker = dict(size = 10, 
...
def plot_weekly_start_times(activity_df, start_year, end_year, work_hours, title_descr):
    ...
    #Load or generate the dta for activity start times across an entire year
    try: 
        ...
        for year in range(pd.DatetimeIndex(activity_df.start_date_local).year.min(), pd.DatetimeIndex(activity_df.start_date_local).year.max() + 1):
            weekly_start_times = generate_weekly_start_time_dict(activity_df, year)
            summary_week_start_times[str(year)] = weekly_start_times
        json.dump(summary_week_start_times, open(os.path.join(os.getcwd(), 'data', 'yearly_week_start_times.json'), 'w'))
...
def plot_eda_data(input_data, year_range, y_label, group_by):
    ...
    data = input_data[(pd.to_datetime(input_data.start_time) > (str(year_range[0])+'-1-1')) & (pd.to_datetime(input_data.start_time) < (str(year_range[1])+'-1-1')) ]#and (pd.to_datetime(input_data.start_time) < pd.to_datetime(str(year_range[1]) + '-1-1'))]
